...
unsafe extern "C" fn yajl_object_free(mut v: yajl_val) {
    let mut i: usize = 0;
    if !(!v.is_null() && (*v).type_0 as c_uint == yajl_t_object) {
        return;
    }
    i = 0;
...
unsafe extern "C" fn yajl_array_free(mut v: yajl_val) {
    let mut i: usize = 0;
    if !(!v.is_null() && (*v).type_0 as c_uint == yajl_t_array) {
        return;
    }
    i = 0;
...
unsafe extern "C" fn context_push(mut ctx: *mut context_t, mut v: yajl_val) -> c_int {
...
unsafe extern "C" fn context_add_value(mut ctx: *mut context_t, mut v: yajl_val) -> c_int {
    if ((*ctx).stack).is_null() {
        (*ctx).root = v;
        0 as c_int
    } else if !((*(*ctx).stack).value).is_null()
        && (*(*(*ctx).stack).value).type_0 as c_uint == yajl_t_object
    {
        if ((*(*ctx).stack).key).is_null() {
            if !(!v.is_null() && (*v).type_0 as c_uint == yajl_t_string) {
                if !((*ctx).errbuf).is_null() {
                    libc::snprintf(
                        (*ctx).errbuf,
                        (*ctx).errbuf_size,
        ...
        && (*(*(*ctx).stack).value).type_0 as c_uint == yajl_t_array
    {
        ...
        if !((*ctx).errbuf).is_null() {
            libc::snprintf(
                ...
                (*(*(*ctx).stack).value).type_0 as c_uint,
            );
        }
        return 22 as c_int;
    }
...
unsafe extern "C" fn handle_string(
    ...
    *((*v).u.string).add(string_length) = 0 as c_int as c_char;
    if context_add_value(ctx as *mut context_t, v) == 0 as c_int {
        1 as c_int
    } else {
        0 as c_int
    }
...
unsafe extern "C" fn handle_number(
    ...
    *((*v).u.number.r).add(string_length) = 0 as c_int as c_char;
    ...
    (*v).u.number.i = yajl_parse_integer(
        (*v).u.number.r as *const c_uchar,
        libc::strlen((*v).u.number.r) as c_uint,
    );
    if get_last_error() == 0 as c_int {
        (*v).u.number.flags |= 0x1;
    }
    endptr = ptr::null_mut::<c_char>();
    set_last_error(0);
    (*v).u.number.d = libc::strtod((*v).u.number.r, &mut endptr);
    if get_last_error() == 0 as c_int && !endptr.is_null() && *endptr as c_int == 0 as c_int {
...
unsafe extern "C" fn handle_start_map(mut ctx: *mut c_void) -> c_int {
    ...
    (*v).u.object.len = 0;
    if context_push(ctx as *mut context_t, v) == 0 as c_int {
        1 as c_int
    } else {
        0 as c_int
    }
}
unsafe extern "C" fn handle_end_map(mut ctx: *mut c_void) -> c_int {
    let mut v: yajl_val = ptr::null_mut::<yajl_val_s>();
...
unsafe extern "C" fn handle_end_array(mut ctx: *mut c_void) -> c_int {
    let mut v: yajl_val = ptr::null_mut::<yajl_val_s>();
    v = context_pop(ctx as *mut context_t);
    if v.is_null() {
        return 0 as c_int;
...
unsafe extern "C" fn handle_boolean(mut ctx: *mut c_void, mut boolean_value: c_int) -> c_int {
    ...
    v = value_alloc(
        (if boolean_value != 0 {
            yajl_t_true as c_int
        } else {
            yajl_t_false as c_int
        }) as yajl_type,
    );
    if v.is_null() {
        if !((*(ctx as *mut context_t)).errbuf).is_null() {
...
pub unsafe extern "C" fn yajl_tree_parse(
    ...
    yajl_config(handle, yajl_allow_comments, 1 as c_int);
    status = yajl_parse(handle, input as *mut c_uchar, libc::strlen(input));
    status = yajl_complete_parse(handle);
    if status as c_uint != yajl_status_ok {
...
pub unsafe extern "C" fn yajl_tree_get(
    ...
    while !n.is_null() && !(*path).is_null() {
        let mut i: usize = 0;
        let mut len: usize = 0;
        if (*n).type_0 as c_uint != yajl_t_object {
            return 0 as yajl_val;
        }
        len = (*n).u.object.len;
        i = 0;
    ...
    if !n.is_null() && type_0 as c_uint != yajl_t_any && type_0 as c_uint != (*n).type_0 as c_uint {
        n = 0 as yajl_val;
    }
    n
}
...
pub unsafe extern "C" fn yajl_tree_free(mut v: yajl_val) {
    ...
    if !v.is_null() && (*v).type_0 as c_uint == yajl_t_string {
        ...
        libc::free(v as *mut c_void);
    } else if !v.is_null() && (*v).type_0 as c_uint == yajl_t_number {
        libc::free((*v).u.number.r as *mut c_void);
        libc::free(v as *mut c_void);
    } else if !if !v.is_null() && (*v).type_0 as c_uint == yajl_t_object {
    ...
    {
        yajl_object_free(v);
    } else if !if !v.is_null() && (*v).type_0 as c_uint == yajl_t_array {
