...
unsafe extern "C" fn yajl_internal_malloc(mut ctx: *mut c_void, mut sz: usize) -> *mut c_void {
    libc::malloc(sz)
}
unsafe extern "C" fn yajl_internal_realloc(
    mut ctx: *mut c_void,
    mut previous: *mut c_void,
    mut sz: usize,
) -> *mut c_void {
    libc::realloc(previous, sz)
}
unsafe extern "C" fn yajl_internal_free(mut ctx: *mut c_void, mut ptr: *mut c_void) {
    libc::free(ptr);
}

pub unsafe extern "C" fn yajl_set_default_alloc_funcs(mut yaf: *mut yajl_alloc_funcs) {
    (*yaf).malloc =
        Some(yajl_internal_malloc as unsafe extern "C" fn(*mut c_void, usize) -> *mut c_void);
    (*yaf).free = Some(yajl_internal_free as unsafe extern "C" fn(*mut c_void, *mut c_void) -> ());
    (*yaf).realloc = Some(
        yajl_internal_realloc
            as unsafe extern "C" fn(*mut c_void, *mut c_void, usize) -> *mut c_void,
    );
    (*yaf).ctx = ptr::null_mut::<c_void>();
}
