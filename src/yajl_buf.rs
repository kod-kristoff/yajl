...
unsafe extern "C" fn yajl_buf_ensure_available(mut buf: yajl_buf, mut want: size_t) {
    ...
    if ((*buf).data).is_null() {
        ...
        (*buf)
            .data = ((*(*buf).alloc).malloc)
            .expect("non-null function pointer")((*(*buf).alloc).ctx, (*buf).len)
            as *mut libc::c_uchar;
        *((*buf).data)
    ...
    if need != (*buf).len {
        (*buf)
            .data = ((*(*buf).alloc).realloc)
            .expect(
                "non-null function pointer",
            )((*(*buf).alloc).ctx, (*buf).data as *mut libc::c_void, need)
            as *mut libc::c_uchar;
        (*buf).len = need;
    }
}
...
pub unsafe extern "C" fn yajl_buf_alloc(mut alloc: *mut yajl_alloc_funcs) -> yajl_buf {
    let mut b: yajl_buf = ((*alloc).malloc)
        .expect(
            "non-null function pointer",
        )((*alloc).ctx, ::core::mem::size_of::<yajl_buf_t>() as libc::c_ulong)
        as yajl_buf;
    memset(
...
pub unsafe extern "C" fn yajl_buf_free(mut buf: yajl_buf) {
    if !((*buf).data).is_null() {
        ((*(*buf).alloc).free)
            .expect(
                "non-null function pointer",
            )((*(*buf).alloc).ctx, (*buf).data as *mut libc::c_void);
    }
    ((*(*buf).alloc).free)
        .expect(
            "non-null function pointer",
        )((*(*buf).alloc).ctx, buf as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn yajl_buf_append(
    ...
    if len > 0 as libc::c_int as libc::c_ulong {
        ...
        (*buf)
            .used = ((*buf).used as libc::c_ulong).wrapping_add(len) as size_t as size_t;
