 ...
 */
pub fn yajl_tree_parse(input: &[u8], error_buffer: &mut [u8]) -> Result<yajl_val, YajlTreeError> {
    println!("yajl_tree_parse");
    let status = yajl_parse(input);
    todo!("parse")
}

#[derive(Debug)]
pub enum YajlTreeError {
    O,
}
// /**
...
// /* Various convenience macros to check the type o a `yajl_val` */
